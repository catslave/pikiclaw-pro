...
export const CLI_TOOLS: RecommendedCli[] = [
  ...
  },
  {
    ...
    binary: 'openclaw',
    name: 'OpenClaw',
    description: 'Self-hosted AI agent Gateway and CLI — run existing OpenClaw agents, sessions, tools, and channels',
    descriptionZh: '自托管 AI Agent Gateway 与 CLI — 复用 OpenClaw 里的 agent、会话、工具和渠道',
    category: 'dev',
    iconSlug: 'openclaw',
    ...
    install: {
      ...
      darwin: [{ label: 'npm', cmd: 'npm install -g openclaw@latest' }],
      linux:  [{ label: 'npm', cmd: 'npm install -g openclaw@latest' }],
      win:    [{ label: 'npm', cmd: 'npm install -g openclaw@latest' }],
    },
    auth: { type: 'none' },
  },
  {
