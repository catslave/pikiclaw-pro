...
import { SlackChannel, type SlackContext, type SlackMessagePayload } from './channel.js';
...
export class SlackBot extends Bot {
  ...
  protected override onManagedConfigChange(config: Record<string, any>, opts: { initial?: boolean } = {}) {
    ...
    if (nextBotToken && nextBotToken !== this.botToken) {
      ...
      if (!opts.initial) this.log('slack botToken reloaded from setting.json');
    }
    if (nextAppToken && nextAppToken !== this.appToken) {
      this.appToken = nextAppToken;
      if (!opts.initial) this.log('slack appToken reloaded from setting.json');
    }
  }
  ...
  private async handleCommand(text: string, ctx: SlackContext): Promise<boolean> {
  ...
  private async cmdStatus(ctx: SlackContext) {
  ...
  private async cmdHost(ctx: SlackContext) {
  ...
  private async cmdAgent(ctx: SlackContext, args: string) {
    if (!args) {
  ...
  private async cmdModels(ctx: SlackContext, args: string) {
  ...
  private async cmdMode(ctx: SlackContext, args: string) {
  ...
  private async cmdSwitch(ctx: SlackContext, args: string) {
  ...
  private async cmdWorkspaces(ctx: SlackContext, args: string) {
  ...
  private async cmdSessions(ctx: SlackContext, args: string) {
  ...
  private async cmdSkills(ctx: SlackContext) {
  ...
  private async cmdStop(ctx: SlackContext) {
  ...
  private async handleMessage(msg: SlackMessagePayload, ctx: SlackContext) {
