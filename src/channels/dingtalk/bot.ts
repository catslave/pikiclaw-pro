...
export class DingtalkBot extends Bot {
  ...
  constructor() {
    ...
    this.clientId = String(config.dingtalkClientId || process.env.DINGTALK_CLIENT_ID || '').trim();
    this.clientSecret = String(config.dingtalkClientSecret || process.env.DINGTALK_CLIENT_SECRET || '').trim();
    if (!this.clientId || !this.clientSecret) {
      throw new Error('Missing DingTalk credentials. Configure dingtalkClientId and dingtalkClientSecret.');
    }
  }
  ...
  protected override onManagedConfigChange(config: Record<string, any>, opts: { initial?: boolean } = {}) {
    const nextId = String(config.dingtalkClientId || process.env.DINGTALK_CLIENT_ID || '').trim();
    const nextSecret = String(config.dingtalkClientSecret || process.env.DINGTALK_CLIENT_SECRET || '').trim();
    if (nextId && nextId !== this.clientId) {
      this.clientId = nextId;
      if (!opts.initial) this.log('dingtalk clientId reloaded from setting.json');
    }
    if (nextSecret && nextSecret !== this.clientSecret) {
      this.clientSecret = nextSecret;
      if (!opts.initial) this.log('dingtalk clientSecret reloaded from setting.json');
    }
  }
  ...
  private async handleCommand(text: string, ctx: DingtalkContext): Promise<boolean> {
  ...
  private async cmdHost(ctx: DingtalkContext) {
  ...
  private async cmdAgent(ctx: DingtalkContext, args: string) {
    if (!args) {
  ...
  private async cmdModels(ctx: DingtalkContext, args: string) {
  ...
  private async cmdMode(ctx: DingtalkContext, args: string) {
  ...
  private async cmdSwitch(ctx: DingtalkContext, args: string) {
  ...
  private async cmdWorkspaces(ctx: DingtalkContext, args: string) {
  ...
  private async cmdSessions(ctx: DingtalkContext, args: string) {
  ...
  private async cmdSkills(ctx: DingtalkContext) {
  ...
  private async cmdStop(ctx: DingtalkContext) {
  ...
  private async handleMessage(msg: DingtalkMessagePayload, ctx: DingtalkContext) {
    const text = msg.text.trim();
    if (text.startsWith('/') && await this.handleCommand(text, ctx)) return;
    if (!text && !msg.files.length) {
      await ctx.reply('This DingTalk channel currently supports text input only.');
      return;
    }
    ...
    void this.queueSessionTask(session, async () => {
      ...
      this.finishTask(taskId);
      this.log(`dingtalk queue execution failed: ${describeError(error)}`);
    });
  }
  ...
  async run() {
    ...
    try {
      ...
      this.startKeepAlive();
      this.log('✓ DingTalk connected, stream listening — ready to receive messages');
